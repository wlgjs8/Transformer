...
def train(epoch):
    ...
    for batch_index, (images, labels) in enumerate(mnist_training_loader):
        ...
        print('Training Epoch: {epoch} [{trained_samples}/{total_samples}]\tLoss: {:0.4f}\tLR: {:0.6f}'.format(
            ...
            total_samples=len(mnist_training_loader.dataset)
        ))
...
def eval_training(epoch=0, tb=True):
    ...
    if tb:
        writer.add_scalar('Test/Average loss', test_loss / len(mnist_test_loader.dataset), epoch)
        writer.add_scalar('Test/Accuracy', correct.float() / len(mnist_test_loader.dataset), epoch)
